...
class App extends Component {
  ...
  removeTrack = givenId => {
    ...
    const filteredList = trackList.filter(eachTrack => eachTrack.id !== givenId)
    this.setState({trackList: filteredList})
  }

  render() {
    const {trackList, searchInput} = this.state
    const filteredList = trackList.filter(eachTrack =>
      eachTrack.name.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return (
      <div className="app-container">
        <div className="background-container">
          <h1 className="bg-heading">Ed Sheeran</h1>
          <p className="bg-para">Singer</p>
        </div>
          ...
          <div className="header-container">
            ...
            <div className="search-container">
              ...
              <button type="button" className="search-button">
                <FaSearch size={20} color="#ffffff" aria-label="faSearch" />
              </button>
            </div>
          </div>
